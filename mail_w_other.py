...
for i in range(2, tot_rows):
    ...
    if country != "US" and country != "XX" and country != "CA":
        j = j + 1;
        parse_names(i,j);
        ...
        postal_code = ws_export.cell(row=i, column=23).value;
        if postal_code == "00000":
            ws_sendout.cell(row=j, column=7).value = "     ";
        else:
            ws_sendout.cell(row=j, column=7).value = ws_export.cell(row=i, column=23).value;
        # Mailing address (country)
        ws_sendout.cell(row=j, column=8).value = country;

tot_entries = j - 1
print "Created Sendout-Other: ", tot_entries
# Save the workbook
